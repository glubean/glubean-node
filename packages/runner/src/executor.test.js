...
import { SHARED_RUN_DEFAULTS } from "./config.js";
...
function getStepStarts(events) {
    return events.filter((e) => e.type === "step_start");
}
function getStepEnds(events) {
    return events.filter((e) => e.type === "step_end");
}
function getWarnings(events) {
    return events.filter((e) => e.type === "warning");
}
function getSchemaValidations(events) {
    return events.filter((e) => e.type === "schema_validation");
}
...
test("TestExecutor - handles missing test", async () => {
    const testFile = await makeTempFile(TEST_FILE_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "nonExistentTest", { vars: {}, secrets: {} });
...
test("TestExecutor - onEvent callback streams events", async () => {
    ...
    const result = await executor.execute(`file://${testFile}`, "passingTest", { vars: {}, secrets: {} }, {
        onEvent: (event) => {
...
test("TestExecutor - executeMany includes testId in streamed events", async () => {
    ...
    const executor = new TestExecutor();
    const streamedEvents = [];
    const batchResult = await executor.executeMany(`file://${testFile}`, ["passingTest", "tracingTest"], { vars: {}, secrets: {} }, {
        concurrency: 2,
        onEvent: (event) => {
            streamedEvents.push(event);
        },
...
test("ctx.fail - immediately aborts test with failure", async () => {
    ...
    expect(result.success).toBe(false);
    const assertions = getAssertions(result.events);
    const failAssertion = assertions.find((a) => a.message === "Something went wrong" && a.passed === false);
...
test("ctx.fail - can be caught in try/catch (user choice)", async () => {
    const testFile = await makeTempFile(FAIL_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "failInTryCatch", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const assertions = getAssertions(result.events);
    const passedAssertion = assertions.find((a) => a.message === "continued after caught fail" && a.passed === true);
...
test("ctx.pollUntil - silent timeout with onTimeout", async () => {
    const testFile = await makeTempFile(POLL_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "pollSilentTimeout", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const logs = result.events.filter((e) => e.type === "log");
...
test("ctx.pollUntil - retries through errors", async () => {
    const testFile = await makeTempFile(POLL_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "pollErrorRetry", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const logs = result.events.filter((e) => e.type === "log");
...
test("ctx.pollUntil - onTimeout receives last error", async () => {
    const testFile = await makeTempFile(POLL_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "pollTimeoutWithError", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const logs = result.events.filter((e) => e.type === "log");
...
test("ctx.setTimeout - can extend timeout dynamically", async () => {
    ...
    const result = await executor.execute(`file://${testFile}`, "extend-timeout", { vars: {}, secrets: {} }, { timeout: 500 });
    expect(result.success).toBe(true);
    const assertions = getAssertions(result.events);
...
test("ctx.setTimeout - can reduce timeout dynamically", async () => {
    const testFile = await makeTempFile(TIMEOUT_UPDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "shorten-timeout", { vars: {}, secrets: {} }, { timeout: 5000 });
    expect(result.success).toBe(false);
    expect(result.error).toBeDefined();
...
test("ctx.setTimeout - ignores invalid timeout updates", async () => {
    const testFile = await makeTempFile(TIMEOUT_UPDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "invalid-timeout-update", { vars: {}, secrets: {} }, { timeout: 100 });
    expect(result.success).toBe(false);
    expect(result.error).toBeDefined();
...
test("builder without .build() is auto-resolved by runner", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "auto-build-test", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
...
test("builder with .build() still works as before", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "explicit-build-test", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
...
test("step retries - passes on retry and continues flow", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-retry-pass", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    ...
    const logs = result.events.filter((e) => e.type === "log");
    expect(logs.some((l) => l.message.includes('Retrying step "flaky with retry" (2/3)'))).toBe(true);
});
test("step retries - exhausted retries fail the step", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-retry-exhausted", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const ends = getStepEnds(result.events);
    expect(ends.length).toBe(2);
...
test("step timeout - marks timed out step as failed", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-timeout-fail", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const ends = getStepEnds(result.events);
    expect(ends.length).toBe(2);
...
test("step timeout - is terminal even when retries are configured", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-timeout-terminal", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const ends = getStepEnds(result.events);
    ...
    expect(ends[0].error?.includes("timed out after 80ms")).toBe(true);
    const logs = result.events.filter((e) => e.type === "log");
    expect(logs.some((l) => l.message.includes('Retrying step "timeout terminal"'))).toBe(false);
});
// =============================================================================
...
test("step events - all passing steps emit correct events", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "steps-all-pass", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
...
test("step events - failed assertion stops subsequent steps", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-assert-fail", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const ends = getStepEnds(result.events);
...
test("step events - thrown error stops subsequent steps", async () => {
    const testFile = await makeTempFile(AUTO_BUILD_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "step-throw-fail", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const ends = getStepEnds(result.events);
    expect(ends.length).toBe(2);
...
test("ExecutionResult - assertionCount and failedAssertionCount", async () => {
    const testFile = await makeTempFile(TEST_FILE_CONTENT);
    const executor = new TestExecutor();
    const passing = await executor.execute(`file://${testFile}`, "passingTest", { vars: {}, secrets: {} });
    expect(passing.assertionCount).toBe(1);
    expect(passing.failedAssertionCount).toBe(0);
    const failing = await executor.execute(`file://${testFile}`, "failingTest", { vars: {}, secrets: {} });
...
test("summary event - includes assertion and step counts", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "steps-all-pass", { vars: {}, secrets: {} });
    const summaries = result.events.filter((e) => e.type === "summary");
...
test("ctx.warn - warning-only test still passes", async () => {
    const testFile = await makeTempFile(TEST_FILE_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "warningOnlyTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const warnings = getWarnings(result.events);
...
test("ctx.validate - passes with valid data (severity: error)", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "validatePassTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
...
test("ctx.validate - fails with invalid data (severity: error)", async () => {
    const testFile = await makeTempFile(VALIDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "validateErrorTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const validations = getSchemaValidations(result.events);
    expect(validations.length).toBe(1);
...
test("ctx.validate - warn severity does not fail the test", async () => {
    const testFile = await makeTempFile(VALIDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "validateWarnTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const validations = getSchemaValidations(result.events);
    expect(validations.length).toBe(1);
    expect(validations[0].success).toBe(false);
...
test("ctx.validate - fatal severity aborts test", async () => {
    const testFile = await makeTempFile(VALIDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "validateFatalTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
    const validations = getSchemaValidations(result.events);
    expect(validations.length).toBe(1);
    expect(validations[0].success).toBe(false);
    expect(validations[0].severity).toBe("fatal");
    const logs = result.events.filter((e) => e.type === "log" && "message" in e && e.message.includes("after fatal"));
    expect(logs.length).toBe(0);
});
test("ctx.validate - parse fallback (no safeParse)", async () => {
    const testFile = await makeTempFile(VALIDATE_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "validateParseFallbackTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    expect(result.failedAssertionCount > 0).toBe(true);
    const validations = getSchemaValidations(result.events);
...
test("HTTP schema - query validation passes with valid params", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "httpQuerySchemaPassTest", { vars: {}, secrets: {} });
    const validations = getSchemaValidations(result.events);
...
test("HTTP schema - query validation fails with invalid params", async () => {
    const testFile = await makeTempFile(HTTP_SCHEMA_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "httpQuerySchemaFailTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    expect(result.failedAssertionCount > 0).toBe(true);
    const validations = getSchemaValidations(result.events);
    const queryValidation = validations.find((v) => v.label === "query params");
...
test("HTTP schema - request body validation passes", async () => {
    const testFile = await makeTempFile(HTTP_SCHEMA_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "httpRequestSchemaTest", { vars: {}, secrets: {} });
    const validations = getSchemaValidations(result.events);
...
test("HTTP schema - request body validation fails", async () => {
    const testFile = await makeTempFile(HTTP_SCHEMA_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "httpRequestSchemaFailTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    expect(result.failedAssertionCount > 0).toBe(true);
    const validations = getSchemaValidations(result.events);
    const bodyValidation = validations.find((v) => v.label === "request body");
...
test("HTTP schema - severity: warn does not fail test", async () => {
    const testFile = await makeTempFile(HTTP_SCHEMA_TEST_CONTENT);
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "httpSchemaWithSeverityTest", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const validations = getSchemaValidations(result.events);
    const queryValidation = validations.find((v) => v.label === "query params");
    expect(queryValidation).toBeDefined();
...
test("executeMany - stopOnFailure stops after first failure", async () => {
    ...
    const executor = new TestExecutor();
    const batch = await executor.executeMany(`file://${testFile}`, ["test1", "test2", "test3", "test4", "test5"], { vars: {}, secrets: {} }, { stopOnFailure: true });
...
test("executeMany - failAfter:2 stops after 2 failures", async () => {
    const testFile = await makeTempFile(FAILFAST_TEST_CONTENT);
    const executor = new TestExecutor();
    const batch = await executor.executeMany(`file://${testFile}`, ["test1", "test2", "test3", "test4", "test5"], { vars: {}, secrets: {} }, { failAfter: 2 });
    expect(batch.success).toBe(false);
...
test("executeMany - no failAfter runs all tests", async () => {
    const testFile = await makeTempFile(FAILFAST_TEST_CONTENT);
    const executor = new TestExecutor();
    const batch = await executor.executeMany(`file://${testFile}`, ["test1", "test2", "test3", "test4", "test5"], { vars: {}, secrets: {} });
    expect(batch.success).toBe(false);
    expect(batch.failedCount).toBe(2);
...
test("executeMany - failAfter:1 is same as stopOnFailure", async () => {
    const testFile = await makeTempFile(FAILFAST_TEST_CONTENT);
    const executor = new TestExecutor();
    const batch = await executor.executeMany(`file://${testFile}`, ["test1", "test2", "test3", "test4", "test5"], { vars: {}, secrets: {} }, { failAfter: 1 });
    expect(batch.success).toBe(false);
    expect(batch.failedCount).toBe(1);
    expect(batch.results.length).toBe(2);
    expect(batch.skippedCount).toBe(3);
...
test("system env fallback - ctx.vars.require reads from system env", async () => {
    ...
    try {
        const result = await executor.execute(`file://${testFile}`, "sysEnvVarTest", { vars: {}, secrets: {} });
        expect(result.success).toBe(true);
        const assertions = getAssertions(result.events);
        expect(assertions.length).toBe(1);
        expect(assertions[0].passed).toBe(true);
...
test("system env fallback - ctx.secrets.require reads from system env", async () => {
    ...
    try {
        const result = await executor.execute(`file://${testFile}`, "sysEnvSecretTest", { vars: {}, secrets: {} });
        expect(result.success).toBe(true);
        const assertions = getAssertions(result.events);
        expect(assertions.length).toBe(1);
        expect(assertions[0].passed).toBe(true);
...
test("system env fallback - .env takes precedence over system env", async () => {
    ...
    try {
        const result = await executor.execute(`file://${testFile}`, "sysEnvOverrideTest", { vars: { GLUBEAN_TEST_OVERRIDE: "from_dotenv" }, secrets: {} });
        expect(result.success).toBe(true);
        const assertions = getAssertions(result.events);
        expect(assertions.length).toBe(1);
        expect(assertions[0].passed).toBe(true);
...
test("system env fallback - ctx.vars.get also falls back", async () => {
    ...
    process.env["GLUBEAN_TEST_SYSENV"] = "from_system";
    try {
        const result = await executor.execute(`file://${testFile}`, "sysEnvGetTest", { vars: {}, secrets: {} });
        expect(result.success).toBe(true);
        const assertions = getAssertions(result.events);
        expect(assertions.every((a) => a.passed)).toBe(true);
    }
...
test("test.extend() - lifecycle fixtures wrap test execution", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "lifecycleFixture", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const assertions = getAssertions(result.events);
...
test("test.extend() - fixtures work with builder API steps", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "builder-fixture", { vars: { BASE_URL: "https://test.api.com" }, secrets: {} });
    expect(result.success).toBe(true);
    const assertions = getAssertions(result.events);
    expect(assertions.length).toBe(1);
    expect(assertions[0].passed).toBe(true);
...
test("test.extend() - lifecycle fixture that skips use() fails the test", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "noUseFixture", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
...
test("test.extend() - lifecycle fixture that calls use() twice fails the test", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "doubleUseFixture", { vars: {}, secrets: {} });
    expect(result.success).toBe(false);
...
test("test.extend() - use() not awaited still completes test body before summary", async () => {
    ...
    const executor = new TestExecutor();
    const result = await executor.execute(`file://${testFile}`, "notAwaitedFixture", { vars: {}, secrets: {} });
    expect(result.success).toBe(true);
    const assertions = getAssertions(result.events);
    expect(assertions.length).toBe(1);
    expect(assertions[0].passed).toBe(true);
